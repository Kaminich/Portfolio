...
export default function Skill({ isDark }: ThemeButtonProps) {
    ...
    return (
        <div>
            <div
                className={`rounded-2xl transition-colors duration-300 flex flex-col align-start gap-5`}
            >
                ...
                <div style={{ flex: 1 }}>
                    ...
                    <div className="grid grid-cols-5 sm:grid-cols-3 md:grid-cols-5 gap-6">
                        {favouriteTools.map((tool) => (
                            <motion.div
                                initial={{ opacity: 0, y: 50 }}
                                whileInView={{ opacity: 1, y: 0 }}
                                transition={{ duration: 0.8 }}
                                viewport={{ once: false, amount: 0.3 }} key={tool.name}>
                                <div
                                    className={`
                                            flex flex-col items-center p-4 rounded-xl transition-all duration-300 shadow-md
